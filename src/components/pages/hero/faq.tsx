...
function HeroFAQ() {
  ...
  return (
    <div
      className="flex flex-col md:!flex-row justify-between items-start container w-full p-5 sm:!p-10 mx-5 sm:!mx-10 mb-10"
      ref={ref1}
      style={{
        ...
        {faq.map((faq, index) => {
          return (
            <AccordionItem key={index} value={`${faq.id}`}>
              <AccordionTrigger>{t(`items.${faq.id}.question`)}</AccordionTrigger>
              <AccordionContent>
