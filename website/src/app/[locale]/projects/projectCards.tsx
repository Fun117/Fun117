...
export const cards: projectExpandableCardGridType[] = [
    ...
    content: () => {
      return (
        <>
          <p>
            制作経緯は、Scratch
            サービスが利用できない時に、サービスの稼働状況を把握したいと思ったからです。
          </p>
          <p>
            Scratch サービスが提供する API を利用し、ユーザーが分かりやすい UI
  ...
  },
  {
    ...
    content: () => {
      return (
        <>
          <p>
            制作経緯は、お問い合わせフォームをウェブサイトに実装したいと思ったことが始まりです。
          </p>
          <p>
            お問い合わせフォームなどを提供するサービスは多く存在しますが、制限や、カスタマイズがあまりできないなどの問題があります。そこで、Google
      ...
      return (
        <>
          <p>
            ...
            のデータパックを配布しようと思ったのが始まりです。
          </p>
          <p>
            人気配布サイトに投稿するのは簡単だけど、不要な機能がたくさんあって面倒だったのもあるけど、自分が好きなデザインではなかったので、前回制作した
    ...
    content: () => {
      return (
        <>
          ...
          <p>
            人気配布サイトに投稿するのは簡単だけど、不要な機能がたくさんあって面倒だったのもあるけど、自分が好きなデザインではなかったので、自分で配布サイトを作成することにしました。
          </p>
          <p>制作期間: 約5時間</p>
        </>
      );
  ...
  },
  {
    ...
    description: () =>
      "Scratch Auth は、Scratch 用のシンプルな OAuth サービスです。開発者にはわかりやすい API を提供し、ユーザーにはスムーズなログイン体験を提供します。Scratch Auth を使用すると、Web サイトに OAuth 機能を簡単に実装できます。",
    content: () => {
      return (
        <>
          <p>
            制作経緯は、過去に制作した Scratch Auth React
            をより強化したパッケージを制作しようと思ったからです。
          </p>
          <p>
            Scratch Auth API
            はとても複雑で実装するのが面倒なので、scratch-auth.config
  ...
  },
  {
    ...
    content: () => {
      return (
        <>
          <p>
            制作経緯は、ウェブサイトを開発するときに毎回テーマ切り替え機能やローカリゼーションのサポートする機能を実装するのが面倒に思って作成に至りました。
          </p>
          <p>
            テンプレートは定期的にアップデートを行い、常に最新のバージョンをサポートするようにしています。
          </p>
          <p>制作期間: 約10時間</p>
        </>
      );
  ...
  },
  {
    ...
    content: () => {
      return (
        <>
          <p>
            制作経緯は、作業ディレクトリーへの移動コマンドや、その他不要ファイルの削除コマンドなどを作成しているときに管理ツールがほしいと思ったのが始まりです。
          </p>
          <p>最低限の機能だけを実装しました。</p>
    ...
    content: () => {
      return (
        <>
          <p>
            制作経緯は、Scratch
            アカウントでのログインする機能を簡単に実装できるようにしたいと思ったからです。
          </p>
          <p>
            パッケージでは、最低限の設定をするだけで、複雑な Scratch Auth の API
            を使った認証機能を実装できるように工夫をして制作をしました。現在はサポートを終了し新たに
            @scratch-auth で同じような機能を提供しています。
          </p>
    ...
    content: () => {
      return (
        <>
          <p>
            制作経緯は、Scratch
            というプラットフォームで公開されていた、人気ゲームの公式ウェブサイトに貢献したいと思ったことが始まりです。
          </p>
          <p>
            初めは 2023年12月16日で、その頃のサイトは HTML
  ...
  },
  {
    ...
    content: () => {
      return (
        <>
          <p>制作経緯は、暗号化という技術に興味をもったことが始まりです。</p>
          <p>
            暗号化は多くの方法がありますが、簡単に暗号化を行えるAPIを作成しようと思い開発をしました。最終的にはキーと内容を指定するだけで、暗号化と復号をすることができるAPIを作成することができました。
          </p>
